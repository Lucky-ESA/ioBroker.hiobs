...
class WebListener extends EventEmitter {
    ...
    onData(data) {
        ...
        try {
            ...
            if (!receive["content"]) {
                this.adapter.log_translator("info", "content_data", JSON.stringify(receive));
                return;
            } else {
    ...
    subscribeHistory(content) {
        if (this.adapter.history && content["dataPoint"] && content["start"] > 0 && content["end"] > 0) {
            this.adapter.sendTo(
    ...
    async createTemplates(content) {
        if (content && content.name != null && content.name != "") {
            ...
            this.adapter.createTemplates(templatename, content);
            const map = {
    ...
    async set_subscribe(data) {
        for (const point of data) {
            ...
            if (this.adapter.subDatapoints[point]) {
                this.adapter.subscribeForeignStates(point);
                map.type = this.adapter.answers.change;
                map.objectID = point;
    ...
    async onLogin(index, devid, content) {
        ...
        if (index != -1 && index != null) {
            ...
            this.deviceID = devid;
            this.adapter.createDevice(devid, content);
            const dev = {
                iob_id: devid,
                device: content["deviceID"].toString(),
                approved: false,
                connected: true,
                key: content["key"] != null ? content["key"].toString() : "",
        ...
        if (content.key != null && user.key === content.key.toString() && user.noPwdAllowed && user.approved) {
            this.isApproved();
        } else if (
            content.user != null &&
        ...
        } else if (content.key != null && user.key !== content.key.toString()) {
