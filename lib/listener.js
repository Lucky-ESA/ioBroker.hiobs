    ...
    async onLogin(index, devid, content) {
        ...
        if (index != -1 && index != null) {
            ...
            if (Object.keys(this.open_notify).length > 0 && this.notNotify) {
                ...
                await this.adapter.setStateAsync(`${this.deviceID}.sendNotification_open`, JSON.stringify([]), true);
            }
        } else {
            this.deviceID = devid;
        ...
        await this.adapter.setStateAsync(`${this.deviceID}.app_version`, content["version"].toString(), true);
        const check_key = await this.adapter.getStateAsync(`${this.deviceID}.aesKey`);
        if (check_key && check_key.val != null && check_key.val != "") {
