...
class WebListener extends EventEmitter {
    ...
    async aes_check() {
        if (this.aesKey == "") {
            ...
            if (check_key && check_key.val != null && check_key.val != "") {
                if (check_key.val.toString().length() > 6) {
                    check_key.val = this.adapter.decrypt(check_key.val.toString());
                }
    ...
    onData(data) {
        ...
        try {
            ...
            if (receive && receive["content"] != null && typeof receive["content"] === "string") {
                if (this.aesKey != "" || receive["type"] === "requestLogin") {
                    this.adapter.log_translator("debug", "key_value", this.aesKey);
                    let aes = "";
                    ...
                    if (this.aesKey == "" && receive["type"] != "requestLogin") {
                        this.adapter.log_translator("debug", "aes_enabled");
                        this.onWrongAesKey();
                        return;
                    }
                }
        ...
        switch (receive["type"].toString()) {
            case "iobStateChangeRequest":
                if (
                    content["stateID"] != null &&
                    this.adapter.subDatapoints[content["stateID"]] != null &&
                    this.adapter.subDatapoints[content["stateID"]].write
                ) {
                    this.adapter.setForeignState(content["stateID"], content["value"], false);
                } else {
                    this.adapter.log.info(`No state change or write false! ${content["stateID"]}`);
                }
                break;
     ...
     */
    async setNotify(content) {
        if (content && content.onlySendNotification != null) {
            this.notNotify = content.onlySendNotification;
        }
    }
    ...
    async sendNotify(content) {
        if (this.notNotify) {
            if (this.ws.isAlive) {
                ...
                if (Object.keys(this.open_notify).length > 0) {
                    for (const message of this.open_notify) {
                        this.sendData(message);
                    }
                    await this.adapter.setState(`${this.deviceID}.sendNotification_open`, {
                        val: JSON.stringify([]),
                        ack: true,
                    });
                }
            } else {
                this.adapter.log_translator("info", "device_offline", this.deviceID);
                const notify = await this.adapter.getStateAsync(`${this.deviceID}.sendNotification_open`);
                try {
                    const open_notify = JSON.parse(notify.val);
                    open_notify.push(content);
                    this.open_notify = open_notify;
                    this.adapter.setState(
                        `${this.deviceID}.sendNotification_open`,
                        JSON.stringify(this.open_notify),
                        true,
                    );
                } catch {
                    this.adapter.log_translator("warn", "cannot_parse");
                }
            }
        } else {
            this.adapter.log.warn(`Notification for device ${this.deviceID} is disabled`);
        }
    }
    ...
    subscribeHistory(content) {
        if (this.adapter.config.history && content["dataPoint"] && content["start"] > 0 && content["end"] > 0) {
            this.adapter.getHistory(
                ...
                (err, result) => {
                    if (err) {
                        this.adapter.log_translator("warn", "Error", err);
                    }
                    if (result) {
                        const map = {
    ...
    async set_subscribe(data) {
        ...
        for (const point of data) {
            ...
            if (this.adapter.subDatapoints[point] && this.adapter.subDatapoints[point].val != null) {
                ...
                map.ack = this.adapter.subDatapoints[point].ack;
                map.ts = this.adapter.subDatapoints[point].ts;
                map.lc = this.adapter.subDatapoints[point].lc;
                sendDP.push(map);
                //this.sendData(map);
            } else {
                ...
                if (obj) {
                    ...
                    subs.write = obj.common && obj.common.write ? obj.common.write : false;
                    subs.read = obj.common && obj.common.read ? obj.common.read : false;
        ...
        if (Object.keys(sendDP).length > 0) {
            const raw = {
                type: this.adapter.answers.answer,
                value: sendDP,
            };
            this.sendData(raw);
        }
    ...
    async onLogin(index, devid, content) {
        ...
        if (index != -1 && index != null) {
            ...
            const notify = await this.adapter.getStateAsync(`${this.deviceID}.sendNotification_open`);
            try {
                this.open_notify = JSON.parse(notify.val);
            } catch {
                this.adapter.log_translator("warn", "cannot_parse");
            }
            if (Object.keys(this.open_notify).length > 0 && this.notNotify) {
                for (const message of this.open_notify) {
                    this.sendData(message);
                }
                await this.adapter.setState(`${this.deviceID}.sendNotification_open`, {
            ...
            const key = await this.adapter.getStateAsync(`${devid}.key`);
            const dev = {
                iob_id: devid,
                device: content["deviceID"].toString(),
                approved: false,
                connected: true,
                key: key != null && key.val != "" ? this.adapter.decrypt(key.val) : "",
        ...
        await this.adapter.setState(`${this.deviceID}.app_version`, { val: content["version"].toString(), ack: true });
        const check_key = await this.adapter.getStateAsync(`${this.deviceID}.aesKey`);
        if (check_key && check_key.val != null && check_key.val != "") {
            if (check_key.val.toString().indexOf("aes-192-cbc") !== -1) {
                check_key.val = this.adapter.decrypt(check_key.val.toString());
            }
            this.aesKey = check_key.val;
        }
        ...
        if (
            ...
            this.adapter.log_translator("info", "key_invalid", this.ip, user.key);
            this.isRejected();
     ...
     */
    async sendData(data) {
        this.adapter.log_translator(
            "debug",
            "send_to",
            `${this.ip} - ${this.ws.isAlive}`,
            this.ip,
            JSON.stringify(data),
        );
        const send = {
                ...
                if (members && members.length > 0) {
                    for (const member of members) {
                        ...
                        if (obj) {
                            ...
                            let name_dp = "";
                            if (typeof obj.common.name === "object" && obj.common.name[this.adapter.lang] != null) {
                                name_dp = obj.common.name[this.adapter.lang];
                            } else {
                                name_dp = obj.common.name;
                            }
                            if (!this.dp_check[member]) {
                                this.dp_check[member] = true;
                                dataPoints.push({
                                    name: name_dp,
                                    id: member,
                                    role: subs.role,
                                ...
                                this.adapter.log.warn(`Found data point ${member}`);
                            }
                        } else {
