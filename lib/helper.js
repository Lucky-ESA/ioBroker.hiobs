module.exports = {
    ...
    async createDevice(id, content) {
        ...
        const key = content["key"] != null && content["key"] != "" ? this.encrypt(content["key"]) : content["key"];
        common = {
        ...
        if (!get_aes || get_aes.val == null || get_aes.val == "") {
            await this.setStateAsync(`${id}.aesKey`, random_key, true);
        } else {
            this.log.warn("Cannot find AES Key. Please Restart Adapter!");
        }
        common = {
        ...
        await this.adapter.subscribeStatesAsync(`${id}.aesKey_new`);
        await this.adapter.subscribeStatesAsync(`${id}.aesKey_active`);
    },
    /**
     * @param {string} id
     */
    async setNewStates(id, content) {
        ...
        content["key"] =
            content["key"] != null && content["key"] != "" ? this.decrypt(content["key"].toString()) : null;
        this.setState(`${id}.key`, content["key"], true);
        content["deviceID"] =
